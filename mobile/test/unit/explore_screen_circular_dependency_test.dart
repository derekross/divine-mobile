...
void main() {
  group('ExploreScreen Circular Dependency Prevention', () {
    test('onScreenHidden uses videoManagerProvider.notifier directly (regression test)', () {
      ...
      const oldImplementationPattern = 'ref.read(exploreVideoManagerProvider)';
      const fixedImplementationPattern = 'ref.read(videoManagerProvider.notifier)';

      // Verify we're not using the problematic pattern in onScreenHidden
      expect(oldImplementationPattern, isNot(equals(fixedImplementationPattern)));
    ...
    test('_onTabChanged uses videoManagerProvider.notifier directly (regression test)', () {
