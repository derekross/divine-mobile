...
void main() {
  group('HashtagFeedScreen TDD Tests', () {
    ...
    testWidgets('should show "Fetching from relays" message when loading hashtag videos', 
    (WidgetTester tester) async {
      ...
      expect(find.text('Be the first to post a video with this hashtag!'), findsNothing);
    });
    ...
    testWidgets('should show videos when hashtag subscription returns results', 
    (WidgetTester tester) async {
      // Given: Videos available for hashtag - use simple test data
      final testVideos = <VideoEvent>[];
      
      when(mockVideoEventService.isLoading).thenReturn(false);
      when(mockHashtagService.getVideosByHashtags(['comedy']))
    ...
    testWidgets('should show "No videos found" only after loading completes with no results', 
    (WidgetTester tester) async {
