...
void main() {
  ...
  group('Video Feed Prefetching', () {
    test('SPEC: should prefetch videos after current index when page changes', () async {
      // Arrange
      ...
      // Assert
      final captured = verify(mockCacheManager.preCache(captureAny, captureAny)).captured;
    ...
    test('SPEC: should prefetch videos before current index when page changes', () async {
      // Arrange
      final videos = _createMockVideoEvents(10);
      ...
      // Assert
      verify(mockCacheManager.preCache(
    ...
    test('SPEC: should not prefetch beyond feed boundaries', () async {
      ...
      expect(actualVideoIds, containsAll(expectedVideoIds));
      expect(actualVideoIds.length, lessThanOrEqualTo(AppConstants.preloadBefore));
    });
    ...
    test('SPEC: should skip already cached videos during prefetch', () async {
      ...
      for (int i = 0; i < videos.length; i++) {
        when(mockCacheManager.isVideoCached(videos[i].id))
            .thenAnswer((_) async => i == 5);
      }
      when(mockCacheManager.preCache(any, any)).thenAnswer((_) async {});

      // Act
      await _triggerPrefetchWithCacheCheck(mockCacheManager, videos, currentIndex);
    ...
    test('SPEC: should handle prefetch errors gracefully', () async {
      ...
      when(mockCacheManager.preCache(any, any))
          .thenThrow(Exception('Network error'));

      // Act & Assert - should not throw
      expect(
        () async => await _triggerPrefetch(mockCacheManager, videos, currentIndex),
        returnsNormally,
      );
    });
    ...
    test('SPEC: should use AppConstants for prefetch window', () async {
      ...
      final expectedCount = AppConstants.preloadBefore + AppConstants.preloadAfter;
